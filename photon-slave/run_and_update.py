...
def fetch_data(arguments, **kwargs):
    ...
    lockfile_path = None
    try:
        lock_data = try_command(ssh_command_base + [user_at_host, args.latest_command, args.remote_directory, last_update], "Locking directory on remote host")
        if not lock_data:
        ...
        try_command(["rsync", "-a", "--delete", "-e", "ssh -o StrictHostKeyChecking=no -i {} -p {}".format(args.identity_file, args.port), "{}:{}".format(user_at_host, remote_dir), dest_dir], "Copying data from remote host")
        if init:
